...
function updateSelectedDisplay(id, arr) {
	...
	arr.forEach(function (item) {
		...
		var removeButton = $(
			'<button type="button" class="select2-selection__choice__remove" tabindex="-1" title="Remove item" aria-label="Remove item" aria-describedby="select2-onboarding_columns-container-choice-' +
				item.id +
		...
		removeButton.on("click", function () {
			var removedId = $(this).parent().attr("data-select2-id");
			var index = arr.findIndex(function (item) {
				return item.id === removedId;
			});
			if (index !== -1) {
				arr.splice(index, 1);
				updateSelectedDisplay(id, arr); // Update display after removal
			}
		});
...
$("#onboarding_columns")
	.on("select2:select", function (e) {
		...
		updateSelectedDisplay("#onboarding_columns", data.onboarding_columns);
	})
