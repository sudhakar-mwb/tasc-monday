...
function removeButtonHandle(data_id) {
	console.log($(this).attr("key"), data_id);
}
	...
	data[forkey].forEach(function (item) {
		...
		var removeButton = $(
			'<button type="button" key="' +
				forkey +
				'" onclick="removeButtonHandle(item.id)" class="select2-selection__choice__remove" tabindex="-1" title="Remove item" aria-label="Remove item" aria-describedby="select2-onboarding_columns-container-choice-' +
				item.id +
