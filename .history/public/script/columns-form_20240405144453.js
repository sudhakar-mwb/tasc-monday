...
function updateSelectedDisplay(id, forkey) {
	...
	arr.forEach(function (item) {
		...
		var removeButton = $(
			'<button type="button" onclick="e.removeAttribute("data-select2-id")" class="select2-selection__choice__remove" tabindex="-1" title="Remove item" aria-label="Remove item" aria-describedby="select2-onboarding_columns-container-choice-' +
				item.id +
...
function removeButton(select_id, forkey, removeid) {
	const index = data[forkey].findIndex((item) => item.id == removeid);
	if (index != -1) data[forkey].splice(index, 1);
}
...
$("#onboarding_columns")
	.on("select2:select", function (e) {
		...
		updateSelectedDisplay("#onboarding_columns", "onboarding_columns");
	})
