...
$(document).ready(function () {
	...
	$("#onboarding_columns").select2({
		sorter: (data) => {
			console.log(data);
			return data.sort((a, b) => b.text.localeCompare(a.text));
		},
	});
	var selectedOptions = [];

	$("#onboarding_columns").on("select2:select", function (e) {
		var data = $(this).select2("data");
		var selectedOption = e.params.data;

		// Add the newly selected option to the array
		selectedOptions.push(selectedOption);

		// Set the new data with preserved order
		$(this).empty().select2({ data: selectedOptions });
	});
