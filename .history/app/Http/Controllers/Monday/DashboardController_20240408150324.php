...
class DashboardController extends Controller
{
  ...
  public function manageById(Request $request)
  {
    ...
    function view_data($data, $response)
    {
      ...
      function findElementById($name, $data, $trackdata, $key = 'value')
      {
          $columnsid = null;
          foreach ($data as $element) {
              if (isset($element['id']) && $element['id'] === $name) {
      ...
      function dateFormater($dateString)
      {
        ...
        $formattedDate = Carbon::createFromFormat('F j, Y', $dateString);
      ...
      $profession = ucwords(findElementById('Profession', $columns, $trackdata, 'value'));
      $profileStatus = Str::upper(strtoupper(findElementById('Overall Status', $columns, $trackdata, 'label')));
      $requestTracking = ucwords('Request Tracking');
      $name = ucwords($trackdata['name']);
      $hiringType = findElementById('Hiring Type', $columns, $trackdata, 'label');
      $nationality = json_decode(findElementById('Country of Residency', $columns, $trackdata, 'value'), true) ?? '';
      $nationality = ($nationality['countryName'] ?? '') . ' ' . ($countryInfo[$nationality['countryCode']]['flag'] ?? '');
      $countryOfResidence = json_decode(findElementById('Nationality', $columns, $trackdata, 'value'), true);
      $countryOfResidence = ($countryOfResidence['countryName'] ?? '') . ' ' . ($countryInfo[$countryOfResidence['countryCode']]['flag'] ?? '');
      $updatesMsg = json_decode(findElementById('Updates', $columns, $trackdata, 'value'), true) ?? '';
      $whatsappNumber = findElementById('Candidate Contact Number (Whatsapp Number)', $columns, $trackdata, 'value');
      $VisaIssuanceValue = json_decode(findElementById('Visa Issuance', $columns, $trackdata, 'value'), true);
      $VisaIssuancestatus = findElementById('Visa Issuance', $columns, $trackdata, 'label');
      ...
      $joiningDate = findElementById('Joining Date', $columns, $trackdata, 'value');
      if ($joiningDate !== null) {
      ...
      $data['overall_status']=$profileStatus;
     for($i=0; $i< count($data['onboarding_columns']);$i++){
      $data['onboarding_columns'][$i]['value']=findElementById()
     }
